...
class moduleGeneral extends miModule
{
	...
	public function __construct()
	{
		...
		$this->check_logger();
		MI()->listen('system/whistle/log', array($this, 'logger') );
...
//		$output = simpleTemplate::parse($template, $tpl);		
	...
	public function debug_page() 
	{
		...
		if ($log_active) 
		{
			...
			$log_file = \file_get_contents ( trailingslashit($upload['basedir']) . 'debug.log' ); 
	...
	public function check_logger() 
	{
		...
		if (isset($settings['log_active']) && $settings['log_active'] == 1) 
		{
			...
			$upload = wp_upload_dir() ;
			$file = fopen( trailingslashit($upload['basedir']) . 'debug.log', $filemode);
			$this->log_handle = $file; 
	...
	public function logger($log)
	{
		...
		if ( is_array($args) )
		{
			foreach($args as $argument) 
			{
				...
				{
					$line_args[] = '<span class="arg">' . $argument . '</span>'; 
				}
			}
			$args = implode("",$line_args); 
			
		}
		else
			$args = '<span class="arg">' . $arg . '</span>'; 


		$line = "<span class='time'>[" . date('d M/Y H:i:s') . "]</span> <span class='message'>" . $msg . "</span> $args " . PHP_EOL; 
