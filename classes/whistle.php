...
class whistle
{
	...
	{
		$this->listen('system/whistle/log_active', array($this, 'toggle_log'), 'tell');
		$this->time_start = microtime(true);
 
	}
	

	public function toggle_log($status)
	{
		$this->log_active = $status;
	}
	
	public function toggle_timer($status) 
	{
		$this->timer_active = $status;
	}
	...
	*/ 
	...
	{
		if ($this->log_active && strpos($msg, 'whistle') === false) 
		{
				$this->tell('system/whistle/log', array($msg, $args),1 ); 
		}
	
	}
	
	public function timer($msg, $args)
	{
		if ( $this->timer_active && strpos($msg, 'whistle') === false )
		{
			$time = microtime(true);
			$elaps = $time - $this->time_start; 
			//$this->log( $msg, array_merge($args,array($elaps)) );
			$this->tell('system/whistle/log', array($msg, array_merge($args,array($elaps)) ),1 ); 
